<?php namespace Frontend\Forum\Classes;
...
class TopicTracker
{
    ...
    public function getNewTopics($member)
    {
        ...
        $results = Db::table('forum_topics')->select([
                'forum_topics.id',
                'forum_topics.channel_id',
                'forum_topics.last_post_at'
            ])
            ->join('forum_channels', 'forum_channels.id', '=', 'forum_topics.channel_id')
            ->where('forum_topics.last_post_at', '>', $lastLogin)
            ->get()
        ;
